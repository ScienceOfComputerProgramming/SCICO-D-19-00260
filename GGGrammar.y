  ...
  | str "=>" ptps ":" str
    ...
    | "choose" exp "{" Branches "}"
      { \cp ->
          let (p, fname, sorts) = $2
              (cp', tagMap) = ($4 cp)
              (ptps, funs) = (updBranches (p, fname, sorts) $4)
...
ptps : str
  ...
  | ptps "," str
    { if (isPtp $3) then $1 ++ [$3] else myErr ("Bad name " ++ $1) }


exp :: { (Ptp, Fname, [Sort]) }
exp : str "(" sorts ")" "@" str
  { if (isPtp $6) then ($5, $1, $3) else myErr ("Bad name " ++ $6) }
...
mergeFuns f g =
  let mergePtp p =
        case (p € M.keys f, p € M.keys g) of
          (False, False) -> []    -- This case never occurs
          (True, False) -> f!p
          (False, True) -> g!p
          (True, True)  -> (f!p) ++ (f!g)
  in L.foldr (\p -> M.fromList (p, mergePtp p)) M.empty ((M.keys f) ++ (M.keys g))
